...
Create a DNSSEC key that will be used to restrict DNS updatesre. This key file and “key name” will be parameters used in the step 5 – “setup ddns_config.yaml”.  Note that DNS Security Extensions (DNSSEC) is a specification that aims at maintaining the data integrity of DNS responses. DNSSEC signs all the DNS resource records (A, MX, CNAME etc.) of a zone using PKI (Public Key Infrastructure).
